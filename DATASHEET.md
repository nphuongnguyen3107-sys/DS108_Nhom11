...
Bộ dữ liệu đặc trưng (`*_features.csv`) bao gồm **33 biến đầu vào** được sử dụng cho huấn luyện mô hình. Biến mục tiêu `pm25` (đã scale) nằm riêng trong các file `*_clean.csv` và được ghép khi huấn luyện.

**Các nhóm đặc trưng bao gồm:** 5 biến khí tượng, 3 biến thời gian, 5 lag features, 6 rolling statistics, 2 difference features, 6 weather interaction features và 6 cyclical encoding features.
